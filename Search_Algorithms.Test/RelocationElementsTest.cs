...
{
	public class RelocationElementsTest
    {
        ...
        {
            RelocationElements.MoveToTail(source, value);
        ...
        {
            Assert.Throws<ArgumentNullException>(() => RelocationElements.MoveToTail(null, 1), "Array cannot be null.");
        }

        [Test]
        public void MoveToTail_ArrayIsEmpty_ThrowArgumentException()
        {
            Assert.Throws<ArgumentException>(() => RelocationElements.MoveToTail(Array.Empty<int>(), 1), "Array cannot be empty.");
        }
    }
}