...
namespace Search_Algorithms.Test
{
	public class FindMaxTests
    {
        ...
        public int FindMaximumItem_ReturnMaximumElement(int[] source) => FindMax.FindMaximum(source);

        [Test]
        public void FindMaximum_ArrayIsNull_ThrowArgumentNullException() =>
            Assert.Throws<ArgumentNullException>(() => FindMax.FindMaximum(null), message: "source cannot be null.");

        [Test]
        public void FindMaximum_ArrayIsEmpty_ThrowArgumentException() =>
            Assert.Throws<ArgumentException>(() => FindMax.FindMaximum(Array.Empty<int>()), message: "source cannot be empty.");

        [Test]
        public void FindMaximum_TestForLargeArray()
        {
            int expected = this.max;
            int actual = FindMax.FindMaximum(this.array);
